...
<head>
    ...
    <link href="${pageContext.request.contextPath}/css/StudAffaDivi.css" rel="stylesheet">
...
<div id="allBody">
    <div class="row border-bottom">
        <nav class="navbar navbar-static-top" role="navigation" style="margin-bottom: 0">
            ...
            <ul class="nav navbar-top-links navbar-right">
                <ul class="menu">
                    <li><a href=""><img src="${sessionScope.admin.adminphoto}" class="touxiangimg" id="touxiaoimg">
                        <span id="loginName" style="height: 30px;font-size: 14px;line-height: 30px">${sessionScope.admin.adminname}</span></a>
                        <ul>
                            <li><a id="selectMyselfOfAdmin">查看个人信息</a>
                            </li>
                            <li><a href="${pageContext.request.contextPath}/admin/StudAffaDivi/StudAffaDiviMain.do">签名盖章管理</a>
                            </li>
    ...
    <div class="row col-md-offset-3" style="position: relative;right: -25px;margin-top: 20px">
        <div class="col-md-7">
            <form role="form" action="${pageContext.request.contextPath}/admin/StudAffaDivi/SelectStudaffadivi.do"
        ...
        <div class="col-md-5">
            ...
            <button class="btn btn-info btn-search" onclick="signAll()">
                <span class="glyphicon glyphicon-save" aria-hidden="true"></span>一键盖章
            </button>
        </div>
    </div>
    <div class="test" style="height: 797px">
        <div class="table">
            ...
            <c:forEach items="${pageInfo.list}" var="StudAffaDivi">
                <div class="body">
                    <div id="stud_id">${StudAffaDivi.studId}</div>
                    <div id="departName">${StudAffaDivi.students.department.departname}</div>
                    <div id="major">${StudAffaDivi.students.major}</div>
                    <div id="sclass">${StudAffaDivi.students.sclass}</div>
                    <div id="grade">${StudAffaDivi.students.grade}</div>
                    <div id="education">${StudAffaDivi.students.education}</div>
                    <div id="studname">${StudAffaDivi.students.studname}</div>
                    <div id="studsex">${StudAffaDivi.students.studsex}</div>
                    <div id="studphone1">${StudAffaDivi.students.studphone1}</div>
                    <div id="studphone2">${StudAffaDivi.students.studphone2}</div>
                    <div id="adress">${StudAffaDivi.students.adress}</div>
                    <div id="familyphone">${StudAffaDivi.students.familyphone}</div>
                    <div id="suppcentsign"><c:if test="${StudAffaDivi.suppcent.admin != 'null'}">
                        <c:out value="${StudAffaDivi.suppcent.admin.adminname}"/></c:if></div>
                    <div id="dormmanasign"><c:if test="${StudAffaDivi.dormmana.admin != 'null'}">
                        <c:out value="${StudAffaDivi.dormmana.admin.adminname}"/></c:if></div>
                    <div id="adminname">${StudAffaDivi.admin.adminname}</div>
                    <div id="signtime"><fmt:formatDate value="${StudAffaDivi.sealtime}" pattern="yyyy-MM-dd HH:mm:ss"/></div>
                    <div>
                        <!--签名按钮触发模态框-->
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="form-horizontal" role="form"
                                  action="${pageContext.request.contextPath}/admin/StudAffaDivi/StudaffadiviSign.do" id="edit">
            ...
            <div class="row">
                ...
                <div class="col-md-5">
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            <li><a href="${pageContext.request.contextPath}/admin/StudAffaDivi/distributionStudaffadivi.do?pn=1">首页</a>
                            </li>
                            <li>
                                <c:if test="${pageInfo.hasPreviousPage}">
                                    <a href="${pageContext.request.contextPath}/admin/StudAffaDivi/distributionStudaffadivi.do?pn=${pageInfo.pageNum-1}"
                                       aria-label="Previous">
                            ...
                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                <c:if test="${page_num==pageInfo.pageNum}">
                                    <li class="active"><a
                                            href="${pageContext.request.contextPath}/admin/StudAffaDivi/distributionStudaffadivi.do?pn=${page_num}">${page_num}</a>
                                    </li>
                                </c:if>
                                <c:if test="${page_num!=pageInfo.pageNum}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/admin/StudAffaDivi/distributionStudaffadivi.do?pn=${page_num}">${page_num}</a>
                                    </li>
                                </c:if>
                            </c:forEach>
                            <li>
                            ...
                            <li>
                                <a href="${pageContext.request.contextPath}/admin/StudAffaDivi/distributionStudaffadivi.do?pn=${pageInfo.pages}">末页</a>
                            </li>
                        </ul>
                    </nav>
                </div>
...
<script>
    $(function () {
        ...
        $(".body").each(function () {
            ...
            btn.bind("click", function () {
                ...
                $('#studentName').val(btn.parent().parent().children("div").get(6).innerHTML);
                $('#adminid').val(loginId);
    ...
    function downloadSuppcenfile() {
        $.ajax({
            url: "${pageContext.request.contextPath}/admin/StudAffaDivi/ExportStudaffadiviFromDB.do",
            type: "POST",
            data: {
                filePath: "123"
            },
        ...
        function return_data(data) {
            ...
            window.location.replace("${pageContext.request.contextPath}/admin/StudAffaDivi/default.do");
        }
    ...
    function signAll() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/admin/StudAffaDivi/StudaffadiviSignAll.do",
            type: "POST",
            data: {
                adminid: adminid
            },
...
<script>
    ...
    function fileLoad(ele){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/admin/StudAffaDivi/ImportStudaffadiviToDB.do",
            type: 'POST',
            data: formData,
            ...
            success: function (responseStr) {
                ...
                $("#textfield").val('');
                $("#importfile").val('');
                window.location.href = "${pageContext.request.contextPath}/admin/StudAffaDivi/default.do";
            }
            ,
