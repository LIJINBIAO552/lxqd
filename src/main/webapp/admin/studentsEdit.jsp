...
<form class="layui-form" action="" style="width: 890px;height: 600px;">
    ...
    <div class="layui-inline">
        <div class="layui-input-block">
            <button class="layui-btn" lay-submit="" lay-filter="editStudents">保存修改</button>
        </div>
    </div>
</form>
...
<script>
    $(function () {
        if ($("#studPhoto").attr("src").length === 0) {
            $("#studPhoto").attr("src","${pageContext.request.contextPath}/img/头像.png");
        }
    })
</script>
<script>
    layui.use(['form', 'layedit'], function () {
        ...
        form.render();
        //监听提交
        form.on('submit(editStudents)', function (data) { //这块要跟表单中的lay-filter="editStudent"对应
            $.ajax({
